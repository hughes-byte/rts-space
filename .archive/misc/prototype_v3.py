# v3. Added credits + Miner ships + asteroid mining loop (go -> mine -> return -> deposit).
# Controls:
#   - Left click: select / box select (same as before)
#   - Left click on empty: move selected ships
#   - Left click on enemy: attack (fighters + miners will "attack" too, you can restrict if you want)
#   - Left click on asteroid: if miner(s) selected => assign mining target
#   - Right click: deselect
#   - Press M: buy miner (costs credits, spawns near your station)
...
MINER_COST = 120
MINING_TIME = 4.0          # seconds to "mine" once landed
MINING_REWARD = 80         # credits per completed run
...
def draw_minimap(surface, asteroids, base, ships, enemy_structures):
    ...
    for s in ships:
        ...
        color = (0, 255, 120) if getattr(s, "kind", "fighter") == "fighter" else (120, 220, 255)
...
def draw_hud(surface):
    global credits
    text = f"Credits: {credits}    (M) Buy Miner: {MINER_COST}"
    img = font.render(text, True, (220, 220, 230))
    surface.blit(img, (14, 14))
...
def draw_ship(surface, screen_pos, angle, thrust=False, scale=0.6, tint=(220, 220, 220)):
...
def asteroid_at_world_point(p_world):
    # returns asteroid index if clicked inside its radius
    for i, (center, r, _tex) in enumerate(asteroids):
        if p_world.distance_to(center) <= r:
            return i
    return None
...
class Ship(GameObject):
    ...
    def update(self, dt, bullets):
        ...
        if do_attack and self.attack_target is not None and self.attack_target.alive:
            ...
            if dist <= self.attack_range and self.fire_cd <= 0:
                ...
                dirv = dirv.normalize() if dirv.length() > 0 else pygame.Vector2(1, 0)
                muzzle = self.pos + dirv * 18
                bullets.append(Bullet(muzzle, dirv * self.bullet_speed, self.bullet_dmg, team="player"))
    ...
    def draw(self, surf, thrusting=False):
        sp = world_to_screen(self.pos)
        tint = (220, 220, 220) if self.kind == "fighter" else (160, 220, 255)
        draw_ship(surf, (sp.x, sp.y), self.angle, thrust=thrusting, scale=self.scale, tint=tint)

        if self.selected:
            draw_health_bar(surf, sp + pygame.Vector2(0, -30), 54, self.hp, self.hp_max)

class MinerShip(Ship):
    def __init__(self, x, y, home_station):
        super().__init__(x, y)
        self.kind = "miner"
        self.hp_max = 90
        self.hp = self.hp_max

        self.home = home_station
        ...
        # mining state
        self.state = "idle"   # idle, to_asteroid, mining, returning
        self.asteroid_i = None
        self.mine_timer = 0.0
        self.cargo = 0

        # make them NOT auto-attack unless you want
        self.attack_target = None
        self.attack_range = 0
        self.fire_rate = 0
        self.bullet_dmg = 0

    def set_mine(self, asteroid_index):
        self.asteroid_i = asteroid_index
        self.state = "to_asteroid"
        center, r, _ = asteroids[asteroid_index]

        # "land" on the asteroid: target a point on the rim toward current position
        dirv = (self.pos - center)
        dirv = dirv.normalize() if dirv.length() > 0 else pygame.Vector2(1, 0)
        land_point = center + dirv * (r + 10)  # just outside surface
        self.set_move(land_point)
    ...
    def update(self, dt, bullets):
        ...
        # mining FSM
        if self.state == "to_asteroid":
            # when move_target cleared => arrived
            thrusting = super().update(dt, bullets)
            if self.move_target is None:
                self.state = "mining"
                self.mine_timer = MINING_TIME
            return thrusting

        if self.state == "mining":
            self.mine_timer -= dt
            # little idle wiggle / rotation while mining
            self.angle += 0.6 * dt
            if self.mine_timer <= 0:
                self.cargo = MINING_REWARD
                self.state = "returning"
                # go home (dock point near station)
                dock = self.home.pos + pygame.Vector2(0, -110)
                self.set_move(dock)
            return False

        if self.state == "returning":
            thrusting = super().update(dt, bullets)
            # deposit when arrived
            if self.move_target is None:
                global credits
                credits += self.cargo
                self.cargo = 0
        ...
        # idle: normal movement if commanded
        return super().update(dt, bullets)
...
class Station(GameObject):
    ...
    def draw(self, surf):
        ...
        if self.selected:
            draw_health_bar(surf, sp + pygame.Vector2(0, -110), 120, self.hp, self.hp_max)
...
# ============================================================
# Spawn player base centered on screen
# ============================================================
...
def selected_units():
    ...
    if base.selected and base.alive:
        out.append(base)
    out.extend([s for s in ships if s.selected and s.alive])
    return out
...
def selected_miners():
    return [s for s in ships if s.selected and s.alive and getattr(s, "kind", "") == "miner"]

def issue_move_command(world_mouse):
    # base move if selected
    if base.selected and base.alive:
...
def issue_attack_command(enemy_obj):
    # Only fighters attack by default (miners won't)
    for s in selected_ships():
        if getattr(s, "kind", "fighter") == "fighter":
            s.set_attack(enemy_obj)

def issue_mine_command(asteroid_index):
    miners = selected_miners()
    if not miners:
        return
    for m in miners:
        m.set_mine(asteroid_index)
...
# ============================================================
def try_buy_miner():
    global credits, ships
    if credits < MINER_COST:
        return

    credits -= MINER_COST

    # spawn near station with small random offset
    ang = random.random() * 2 * math.pi
    r = random.randrange(130, 190)
    spawn = base.pos + pygame.Vector2(math.cos(ang) * r, math.sin(ang) * r)

    miner = MinerShip(spawn.x, spawn.y, base)
    ships.append(miner)
...
while running:
    ...
    for e in pygame.event.get():
        ...
        # Left up: click -> (enemy attack) OR (asteroid mine if miner selected) OR move. Drag -> box select.
            ...
            if selecting:
                ...
                if box.width < 6 and box.height < 6:
                    ...
                    if enemy is not None:
                        ...
                        a_i = asteroid_at_world_point(mouse_world)
                        if a_i is not None and selected_miners():
                            issue_mine_command(a_i)
                        else:
    ...
    for s, thr in zip(ships, thrusting_states):
        ...
        # (optional) show miner state text when selected
        if getattr(s, "kind", "") == "miner" and s.selected:
            sp = world_to_screen(s.pos)
            state = getattr(s, "state", "idle")
            label = font.render(state, True, (160, 220, 255))
            screen.blit(label, (int(sp.x - 24), int(sp.y + 18)))
    ...
    draw_minimap(
        ...
        asteroids,
        base,
        [s for s in ships if s.alive],
        [e for e in enemy_structures if e.alive],
    )
